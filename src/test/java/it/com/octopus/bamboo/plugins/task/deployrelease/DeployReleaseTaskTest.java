...
public class DeployReleaseTaskTest extends BaseTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(DeployReleaseTaskTest.class);
    private final DeployReleaseTask deployReleaseTask;

    @Inject
    public DeployReleaseTaskTest(@ComponentImport @NotNull final DeployReleaseTask createReleaseTask) {
        this.deployReleaseTask = createReleaseTask;

        /*
            Replace some of the injected services with mocks/stubs
         */
        deployReleaseTask.setProcessService(processService);
        deployReleaseTask.setCapabilityContext(stubCapabilityContext);
    }
    ...
    public void testCommands() throws TaskException {
        Assert.assertNotNull(deployReleaseTask);

        deployReleaseTask.execute(mockCommonTaskContext);

        Assert.assertEquals(stubCapabilityContext.getOctoPath(),
                processService.getCommands().get(0));

        Assert.assertEquals(OctoConstants.DEPLOY_RELEASE_COMMAND,
